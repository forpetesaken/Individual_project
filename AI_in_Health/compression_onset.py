...
def main(args):
    ...
    if groups is not None:
        ...
        # trackers for "best fold by mean(AP_comp, AP_reg)"
        best_score  = -1.0
        best_meta   = None   # (fold_idx, ap_comp, ap_reg, best_comp_dict, best_reg_dict)
        best_bundle = None   # dict to joblib.dump at the end
        ...
        for fold, (tr, va) in enumerate(gkf.split(X, y, groups=groups)):
            ...
            # Thresholds (on this fold's val set)
            ...
            if score > best_score:
                ...
                # tie-breaker 1: higher AP on REGULAR
                prev = best_meta
            ...
            if improves:
                ...
                best_bundle = {
                    ...
                    # bookkeeping
                    "selected_by": "mean(AP_comp, AP_reg)",
            ...
            plt.title(f"Fold {fold+1} – loss curve"); plt.grid(True)
        ...
        if best_bundle is None:
            ...
            print(f"AP_comp={best_bundle['ap_comp']:.3f} | AP_reg={best_bundle['ap_reg']:.3f} "
                  f"| meanAP={(best_bundle['ap_comp']+best_bundle['ap_reg'])/2:.3f}")
            print(f"Using thresholds: T_comp={best_bundle['T_comp']:.3f} | T_reg={best_bundle['T_reg']:.3f}")
            print(f"Saved best fold model to: {os.path.abspath(args.out)}")
